...
function containsForbiddenRawPayload(value: unknown): boolean {
  ...
  const forbidden = /^(?:raw(?:care)?(?:record|records|text|payload|notes)|care(?:record|records|notes)|residentnotes|staffrecords|medicationrecords|specialcategorydata|recordpayload)$/;
  for (const [key, nested] of Object.entries(value)) {
    const normalizedKey = key.replace(/[^a-z0-9]/gi, '').toLowerCase();
    if (forbidden.test(normalizedKey)) return true;

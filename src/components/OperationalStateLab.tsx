...
const STATUS_CLASS: Record<OperationalStatus, string> = {
  ...
  DISPUTED: 'text-purple-700 border-purple-500/30 bg-purple-500/10',
};

function controlGlyph(observation: EvaluatedControl) {
  if (observation.effectiveValue === 'yes' && observation.epistemic === 'OBSERVED' && observation.evidenceIds.length > 0) {
    return { label: 'YES', cls: 'text-flag-green bg-flag-green/10 border-flag-green/20' };
  }
  if (observation.disputed) {
    return { label: 'DISPUTED', cls: 'text-purple-700 bg-purple-500/10 border-purple-500/20' };
  }
  if (observation.effectiveValue === 'no') {
    return { label: 'NO', cls: 'text-flag-red bg-flag-red/10 border-flag-red/20' };
  }
  if (observation.effectiveValue === 'partial') {
    return { label: 'PART', cls: 'text-flag-amber bg-flag-amber/10 border-flag-amber/20' };
  }
  if (observation.effectiveValue === 'yes') {
...
export function OperationalStateLab() {
  const snapshot = useMemo(() => evaluateOperationalLedger(buildDemoOperationalState()), []);
  const services = snapshot.ledger.topology.filter(node => node.kind === 'service');
  const statusCounts = snapshot.capabilities.reduce<Record<OperationalStatus, number>>((acc, item) => {
    acc[item.status] += 1;
    return acc;
  }, { VERIFIED: 0, READY: 0, PARTIAL: 0, BLOCKED: 0, UNKNOWN: 0, DISPUTED: 0 });

  const serviceName = (id: string) => snapshot.ledger.topology.find(node => node.id === id)?.name || id;
            ...
            <table className="w-full min-w-[980px] border-separate border-spacing-y-2">
              ...
              <tbody>
                {snapshot.capabilities.map(item => (
                  <tr key={item.record.id} className="bg-hc-surface-2/55">
                    <td className="rounded-l-xl px-3 py-3">
                      <div className="text-[10px] font-black text-hc-text">{item.definition.label}</div>
                      <div className="mt-1 text-[8px] font-bold uppercase tracking-widest text-hc-muted">{serviceName(item.record.serviceId)} · {item.record.sourceSystem}</div>
                    </td>
                    {OPERATIONAL_CONTROL_KEYS.map(key => {
                      const glyph = controlGlyph(item.controls[key]);
                      return (
                        <td key={key} className="px-2 py-3 text-center">
                          <span title={item.controls[key].note || item.controls[key].epistemic} className={`inline-flex min-w-[44px] justify-center rounded-lg border px-2 py-1 text-[8px] font-black uppercase tracking-wider ${glyph.cls}`}>
                            {glyph.label}
                          </span>
                        </td>
                      );
          ...
          <div className="hc-clay-raised rounded-[2rem] p-5">
            ...
            <div className="space-y-3 max-h-[470px] overflow-y-auto pr-1">
              {snapshot.deltas.slice(0, 10).map(delta => (
                <div key={delta.key} className="rounded-xl border border-hc-border/15 bg-hc-surface-2/60 p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="text-[10px] font-black text-hc-text">{delta.capabilityLabel}</div>
                      <div className="text-[8px] font-bold uppercase tracking-widest text-hc-muted mt-1">
                        {serviceName(delta.serviceId)} · {delta.control ? OPERATIONAL_CONTROL_LABELS[delta.control] : delta.requirementId || 'Evidence contract'}
                      </div>
                    </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        ...
        <div className="hc-clay-raised rounded-[2rem] p-5">
          ...
          <div className="space-y-4">
            {snapshot.ledger.contracts.map(contract => (

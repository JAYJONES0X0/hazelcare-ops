...
function NeedLevelSelector({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  ...
  return (
    <div className="mb-10 animate-in fade-in slide-in-from-bottom-4 duration-500 text-hc-text">
      <label className="text-[10px] mb-5 ml-1 block font-black opacity-60 tracking-[0.2em] uppercase">Tactical Support Requirement</label>
      <div className="flex flex-wrap gap-3">
        {LEVEL_OF_NEED_LABELS.map((label, i) => (
          <button key={i} onClick={() => onChange(i)}
            className={`flex-1 text-[10px] font-black uppercase tracking-widest py-4 px-2 rounded-2xl transition-all duration-500 shadow-xl active:scale-95 border
              ${value === i
                ? `hc-clay-inset bg-hc-bg/50 border-hc-teal/30 scale-105 z-10`
                : 'hc-clay-raised border-hc-muted/5 text-hc-muted hover:text-hc-text'
            }`}
...
function RiskScoreWidget({ likelihood, impact, onLikelihood, onImpact }: {
  ...
  return (
    <div className="hc-clay-raised rounded-[2.5rem] p-10 mb-10 shadow-2xl relative overflow-hidden group border border-hc-muted/5">
      ...
      <div className="flex items-center gap-6 pt-6 border-t border-hc-muted/10 relative z-10">
        <span className="text-[10px] font-black text-hc-muted uppercase tracking-[0.2em]">Clinical Risk Score:</span>
        <span className="text-4xl font-black tabular-nums tracking-tighter" style={{ color }}>{score}</span>
        <span className="pill !bg-hc-bg text-[10px] font-black uppercase tracking-[0.2em] px-6 py-1.5 shadow-xl border border-hc-muted/5" style={{ color }}>{label}</span>
...
function DomainEditor({ domain, onChange }: {
  ...
  return (
    <div className="animate-in slide-in-from-right-4 duration-700 text-hc-text">
      <div className="flex items-center gap-8 mb-12 group">
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <p className="text-[11px] font-black text-hc-muted uppercase tracking-[0.3em] opacity-60">Operational Support Parameters</p>
          </div>
        </div>
      </div>
      ...
      <div className="h-10" />
      <div className="text-[10px] font-black mb-6 ml-1 opacity-60 tracking-[0.3em] uppercase">Sector Risk Management</div>
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden animate-in fade-in duration-500 bg-hc-bone">
      ...
      <div className="flex items-center gap-6 px-10 py-6 hc-clay-raised z-20 shadow-2xl relative">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-2xl font-black text-hc-text tracking-tighter uppercase flex items-center gap-4">
            ...
            <span className="pill !bg-hc-bg text-hc-purple border border-hc-purple/30 text-[10px] font-black tracking-widest px-4 py-1 shadow-lg uppercase">Clinical Support Plan</span>
          </h1>
          <div className="flex items-center gap-4 mt-2">
            <span className="text-[10px] font-black text-hc-muted uppercase tracking-[0.2em] opacity-60">Tactical Care Delivery</span>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Domain sidebar */}
        <div className="w-80 flex-shrink-0 border-r border-hc-muted/10 overflow-y-auto hc-clay-raised z-10 scrollbar-thin">
          <div className="p-8 border-b border-hc-muted/10 bg-black/[0.02] flex items-center justify-between">
            <p className="text-[10px] font-black tracking-[0.3em] text-hc-text opacity-40 uppercase">Sector Nodes</p>
            <span className="pill !bg-hc-bg text-[9px] font-black text-hc-teal px-3 py-1 border border-hc-teal/20 tabular-nums">{filledCount}/{enabledCount}</span>
          </div>
          <div className="py-6">

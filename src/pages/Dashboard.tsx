  ...
  return (
    <div className="p-6 lg:p-10 w-full animate-in fade-in duration-1000">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-10">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tighter uppercase text-shimmer">Strategic Dashboard</h1>
          <p className="text-hc-muted text-xs font-bold uppercase tracking-[0.2em] mt-1">Network-Wide Operational Velocity</p>
        </div>
        <div className="flex gap-3">
          <button onClick={() => setPage('briefing')} className="glass-light border border-white/10 text-[10px] font-black uppercase tracking-[0.2em] px-6 py-3 rounded-xl hover:text-white hover:border-hc-teal/40 transition-all shadow-xl active:scale-95">Daily Briefing</button>
          <button onClick={() => setPage('upload')} className="btn-gradient px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl hover:scale-105 active:scale-95">Synchronize Data</button>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-5 gap-6 mb-10">
        {[
          { label: 'Active Personnel', val: '42', sub: 'Nodes deployed', color: '#14b8a6' },
          { label: 'Total Transmissions', val: weekData.totalEntries, sub: 'Temporal cycle', color: '#3b82f6' },
          { label: 'Critical Overdue', val: actions.filter(a => a.priority === 'critical' && a.status !== 'completed').length, sub: 'Immediate attention', color: '#ef4444' },
          { label: 'Open Objectives', val: openActions.length, sub: 'In progress', color: '#f59e0b' },
          { label: 'Active Alerts', val: activeIncidents.length, sub: 'Incident pipeline', color: '#8b5cf6' },
        ].map(s => (
          <div key={s.label} className="glass-light border border-white/5 rounded-[2rem] p-8 card-glow group relative overflow-hidden active:scale-95 transition-all">
            ...
            <div className="text-4xl font-black text-white tabular-nums tracking-tighter mb-2 group-hover:scale-110 transition-transform duration-500" style={{ textShadow: `0 0 20px ${s.color}40` }}>{s.val}</div>
      ...
      <div className="mb-10">
        <div className="flex items-center gap-4 mb-8 px-2">
          ...
          <h2 className="text-2xl font-black text-white tracking-tighter uppercase text-shimmer">Sector Node Readiness</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
          {houseList.map((house, idx) => {
            ...
            return (
              <div
                key={house.name}
                className={`glass-light border rounded-[2rem] p-8 hover:bg-hc-card-hover/40 transition-all duration-500 card-glow group/house animate-in slide-in-from-bottom-4 active:scale-[0.98] shadow-2xl
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  <div>
                    ...
                    {house.coordinator && <div className="text-[9px] text-hc-muted font-bold uppercase tracking-[0.2em] opacity-60">Lead Agent: {house.coordinator}</div>}
                  </div>
                ...
                {/* Mini stats */}
                <div className="grid grid-cols-4 gap-4 mb-8 bg-black/20 p-4 rounded-2xl border border-white/5 shadow-inner">
                  {[
                    { n: house.entries.length, l: 'LOGS', c: '#d4dfe8' },
                    { n: house.incidents.length, l: 'ALERTS', c: '#ef4444' },
                    ...
                    <div key={s.l} className="text-center group/stat cursor-default">
                      <div className="text-xl font-black transition-transform duration-500 group-hover/stat:scale-125 tabular-nums leading-none" style={{ color: s.c }}>{s.n}</div>
                ...
                {/* Severity bar */}
                <div className="h-2.5 rounded-full bg-hc-dark/80 overflow-hidden flex shadow-inner border border-white/5 mb-8">
      ...
      {/* Tactical maneuvers footer */}
      <div className="mt-20 pt-10 border-t border-white/5">
        <h3 className="section-header text-[9px] mb-8 ml-2 opacity-40 tracking-[0.4em]">TACTICAL MANEUVERS</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 pb-10">
          {[
            { id: 'incidents', label: 'Monitor Alerts', desc: 'Active incident pipeline', icon: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4.5c-.77-.833-2.694-.833-3.464 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z' },
            { id: 'actions', label: 'Review Objectives', desc: 'Fleet-wide action tracker', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4' },
            { id: 'staff', label: 'Roster Logic', desc: 'Personnel presence status', icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z' },
            { id: 'reports', label: 'Analytical Suite', desc: 'Advanced pattern mapping', icon: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m0 0a2 2 0 002 2h2a2 2 0 002-2V5a2 2 0 00-2-2h-2a2 2 0 00-2 2v14' },
          ].map(btn => (
            <button
              key={btn.id}

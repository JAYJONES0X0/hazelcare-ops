      ...
      <div className="flex items-center justify-between mb-8">
        ...
        {datasets.some(d => d.present) && (
          <button onClick={() => { if (confirm('Delete ALL data from this device?')) onClearEverything(); }}
            className="text-[9px] font-black text-flag-red hover:text-hc-text uppercase tracking-[0.2em] px-4 py-2 glass-light border border-flag-red/20 rounded-xl transition-all hover:bg-flag-red/20">
            Purge All Data
          </button>
        )}
      </div>
    ...
    <div className="p-6 lg:p-10 w-full animate-in fade-in duration-700 scrollbar-thin max-w-[2560px] mx-auto">
          ...
          <div className="glass border border-hc-teal/30 rounded-[2.5rem] p-8 mb-8 relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-5 mb-8">
                ...
                <div>
                  <h2 className="text-2xl font-black text-hc-text tracking-tighter uppercase text-shimmer">{detectedInfo.label} Identified</h2>
                  ...
                  <div className="glass-light border border-hc-border rounded-2xl p-4 space-y-3 group hover:border-hc-teal/30 transition-all">
                    ...
                    {preview.clientName && (
                      <div className="text-sm text-hc-muted">
                        Detected candidate: <span className="text-hc-text font-bold">{preview.clientName}</span>
                      </div>
                    )}
                    <div className="text-xs text-hc-muted/80 leading-relaxed">
                      Use <span className="text-hc-text font-semibold">Global Import</span> to create/update from the dataset directly.
                      Use <span className="text-hc-text font-semibold">Specific Client</span> when you want to pin import to one person.
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-6 glass-light border border-hc-border rounded-2xl p-4">
                  ...
                  {templateMode === 'specific' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                      {TEMPLATES.map((tpl) => (
                        <label key={tpl.id} className="flex items-center gap-2 text-sm text-hc-text font-bold uppercase tracking-wide">
                          <input
                            type="checkbox"
                    ...
                    <div className="space-y-2">
                      {gaps.map((gap) => (
                        <div key={gap.id} className="text-xs text-hc-muted">
                          <span className="text-hc-text font-semibold">{gap.label}</span> - {gap.why} Suggested source: <span className="text-hc-text">{gap.recommendedSource}</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div className="flex gap-4">
                <button onClick={() => handleConfirm()}
                  className="flex-[2] btn-gradient text-hc-text text-[11px] font-black uppercase tracking-[0.3em] py-5 rounded-2xl shadow-2xl hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3">
      ...
      {step === 'done' && (
        <div className="flex flex-col items-center justify-center py-20 animate-in fade-in zoom-in-95 duration-500">
          ...
          <div className="text-xl font-black text-hc-text mb-2">Import Complete</div>
      ...
      {!!importGaps.length && pendingConfirmBypassGaps && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-2xl glass border border-flag-amber/30 rounded-2xl p-6">
            <div className="text-lg font-black text-hc-text mb-2">More Source Data Recommended</div>
            <div className="text-sm text-hc-muted mb-4">

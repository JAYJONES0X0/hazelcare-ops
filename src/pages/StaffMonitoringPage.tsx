...
export function StaffMonitoringPage({ weekData, onDataParsed, setPage }: Props) {
  ...
  // â”€â”€ Task Note Coaching Letter â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    >
      ...
      <div className="w-full overflow-x-auto scrollbar-none mb-6">
        <div className="inline-flex items-center p-1.5 rounded-[2rem] hc-clay-raised min-w-max">
          ...
          >
            <FileText size={13} /> Open Shift Handovers
          </button>
          <button
            type="button"
            onClick={() => setPage?.('nourish-tasks')}
            disabled={!setPage}
            className="flex items-center gap-2 px-6 py-2.5 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all duration-300 hc-clay-raised text-hc-text disabled:opacity-40"
          >
      ...
      {(activeTab === 'handover' || activeTab === 'task-notes') && (
                                      ...
                                      <div className="space-y-1">
                                        {gap.failureMessages.map((msg, i) => (
                                          <div key={i} className="flex items-start gap-2 text-[10px] text-flag-red/80">
                                            <span className="shrink-0 mt-0.5">â€º</span>{msg}
                                          </div>
                                        ))}
                                      </div>
                                    </div>
        ...
        <div className="w-full xl:w-[500px] shrink-0">
          {/* â”€â”€ Task Note Coaching Studio â”€â”€ */}
                              ...
                              <div className="px-4 py-3 border-b border-hc-border/10 bg-flag-red/5">
                                ...
                                {gap.failureMessages.map((msg, j) => (
                                  <div key={j} className="text-[10px] text-flag-red/80 flex items-start gap-1.5 mb-1">
                                    <span className="shrink-0">â€º</span>{msg}
                                  </div>
                                ))}
                              </div>
                            )}
          ...
          {/* â”€â”€ Handover Coaching Studio â”€â”€ */}
                    ...
                    <div className="space-y-1.5">
                      {coachRecord.topGaps.slice(0, 5).map((gap, i) => (
                        <div key={i} className="flex items-start gap-2 text-[11px] text-hc-muted">
                          <span className="text-flag-amber shrink-0 mt-0.5">â€º</span>
                          {gap}
                        </div>
                      ))}
                    </div>
                            ...
                            <div key={entry.id} className="rounded-2xl overflow-hidden border border-hc-border/20">
                              ...
                              {missing.length > 0 && (
                                <div className="px-4 py-3 border-b border-hc-border/10 bg-flag-red/5">
                                  ...
                                  {missing.map((gap, j) => (
                                    <div key={j} className="text-[10px] text-flag-red/80 flex items-start gap-1.5 mb-1">
                                      <span className="shrink-0">â€º</span>{gap}
                                    </div>
                                  ))}
                                </div>
                              )}

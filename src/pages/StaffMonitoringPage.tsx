...
export function StaffMonitoringPage({ weekData, setPage, onDataParsed }: Props) {
  ...
  const persistedEntries = useMemo(() => {
    if (!temporalRange.from && !temporalRange.to) return null;
  ...
  const onRecompute = useCallback(() => {
    saveMonitoringRun(`${snapshot.windowLabel} Â· ${snapshot.dataFreshness.entryCount} entries`, snapshot.escalations.length);
  ...
  function copyCoachingMessage() {
    ...
    const msg = [
      ...
      `I've been reviewing your recent care entries. You're doing the work Â· I'd just like the notes to reflect that more fully. Moving forward, please:`, ``,
      `â€¢ Write in first person ("I supported...")`, `â€¢ Show your decision-making`, `â€¢ Document presentation changes`, ``,
  ...
  return (
    ...
    >
      ...
      {importError && (
        <div className={`mb-10 px-8 py-5 rounded-[2rem] hc-clay-inset text-[11px] font-black uppercase tracking-widest flex items-center gap-4 animate-in slide-in-from-top-4 duration-500 ${importError.includes('Merged') ? 'text-flag-green' : 'text-flag-red'}`}>
      ...
      {(storeBounds || persistedEntries !== null) && (
        <div className="mb-8 animate-in slide-in-from-top-2 duration-500">
          <DateRangePicker range={temporalRange} onChange={setTemporalRange} entryCount={filteredEntries.length} />
          {persistedEntries !== null && (
            <div className="mt-3 px-4 py-2 bg-hc-teal/5 border border-hc-teal/20 rounded-xl text-[10px] font-black text-hc-teal uppercase tracking-widest flex items-center gap-2">
              ...
              Temporal mode active Â· showing {filteredEntries.length.toLocaleString()} entries from persistent store
              <button onClick={() => setTemporalRange({ from: null, to: null })} className="ml-auto text-hc-muted hover:text-hc-text transition-colors">âœ• reset</button>
            </div>
          )}
        </div>
      )}
          ...
          <div className="flex-1 flex flex-col gap-10">
            <section className="space-y-6">
              <div className="flex items-center gap-3 px-4">
                ...
                <h2 className="text-[11px] font-black text-hc-text uppercase tracking-[0.3em]">Critical Review Â· {snapshot.escalations.length}</h2>
              </div>
                      ...
                      <div className="p-6 flex items-center justify-between cursor-pointer group" onClick={() => setSelectedStaffCard(isExpanded ? null : s.carer)}>
                           ...
                           <div>
                              ...
                              <div className="flex flex-wrap items-center gap-2 mt-1">
                                {s.categoryBreakdown.map(({ category, count }) => (
                                  <span key={category} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest bg-hc-teal/10 text-hc-teal`}>
                        ...
                        <div className="flex items-center gap-10">
                           <div className="flex flex-col items-end gap-1">
                              <span className={`text-[11px] font-black tabular-nums tracking-tighter ${scoreColor}`}>{s.qualityScore}%</span>
                        ...
                        <div className="px-8 pb-8 pt-4 animate-in slide-in-from-top-4 duration-500 flex flex-col gap-6">
                           ...
                           <div className="flex justify-end gap-4">
                              <button onClick={(e) => { e.stopPropagation(); setCoachStaff(s.carer); setCoachEntry(null); setCoachRewrite(''); setSelectedEscId(esc?.id || null); }}
                                className="px-8 py-3.5 rounded-2xl btn-tactical text-[10px] font-black uppercase tracking-widest shadow-xl">Contextual Coaching Studio Â· Summarise Â»</button>
                           </div>
                        </div>
                      )}
                    </div>
            ...
            {coachStaff ? (
              <div className="hc-clay-raised p-8 flex flex-col gap-8 relative overflow-hidden h-fit sticky top-10 animate-in slide-in-from-right-8 duration-700">
                ...
                <div className="space-y-10">
                  <section>
                    <label className="text-[10px] font-black text-hc-muted uppercase tracking-[0.3em] mb-5 block">1. Vector Target Selection</label>
                    <select className="w-full hc-clay-inset px-5 py-4 text-[11px] font-black uppercase tracking-widest text-hc-text outline-none shadow-inner"
                      ...
                      {[...(entriesByStaff[coachStaff] || [])].sort((a, b) => scoreEntry(a).total - scoreEntry(b).total).slice(0, 20).map((e, i) => (
                        <option key={i} value={e.entry}>{e.date} Â· {scoreEntry(e).total}%</option>
                      ))}
                    </select>
                  </section>
          ...
          {loadCallOutcomes().map((o) => (
            <div key={o.id} className="p-6 rounded-2xl hc-clay-inset flex flex-col gap-4 group hover:bg-black/[0.02] transition-all shadow-inner">
              <div className="flex justify-between items-center">
                ...
                <span className="text-hc-muted text-[11px] font-black tabular-nums">{new Date(o.at).toLocaleString('en-GB', {day:'2-digit', month:'2-digit', hour: '2-digit', minute:'2-digit'})}</span>
              </div>
              <div className="flex items-center gap-4">
                <span className="pill !bg-hc-bg text-hc-teal border border-hc-teal/30 text-[10px] shadow-sm">{o.outcome}</span>
                <div className="text-[11px] text-hc-text font-black truncate italic opacity-80 flex-1 uppercase">"{o.notes}"</div>
              </div>
            </div>
          ))}
          {loadCallOutcomes().length === 0 && <div className="text-[11px] font-black text-hc-muted col-span-full text-center py-20 uppercase tracking-[0.4em]">No clinical evidence logged.</div>}
        </div>
      </div>
    </div>
  );
